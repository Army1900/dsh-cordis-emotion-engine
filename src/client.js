/**
 * 情绪引擎 Emotion Engine — Client 半部
 *
 * 本文件是 cordis_define 的 code.client 函数体（纯 JavaScript，无 TS/JSX/import，
 * React 组件一律使用 React.createElement）。
 * 在 DSH 会话中加载时，把本文件内容作为 code.client 传入 cordis_define 即可。
 *
 * 职责：
 *  - 注册 shell.overlay 悬浮组件：半透明水波球 + 磨砂情绪面板
 *  - 每 2.5s 轮询 Host 的 mood:analyze，自动感知当前会话最近输入的情绪
 *  - 通过 theme.overrideTokens 覆盖全局主题 token（含侧边栏），页面随情绪变色
 *  - 渲染情绪专属的全屏动态渐变背景层（低强度 soft-light，避免遮挡文字）
 *  - 情绪变化时调用 mood:set 同步给 Host（用于系统提示词注入）
 */
return {
  inject: ['timer'],
  apply(ctx) {
    ...
    ctx.effect(() => styles.insert(`
      ...
      /* ---- animated full-screen tint (below orb/panel, softened to keep text readable) ---- */
      ...
      /* happy: warm golden clouds drifting (softened) */
      .mood-bg-happy {
        background:
      ...
      /* calm: slow waves flowing (softened) */
      .mood-bg-calm {
        background:
      ...
      /* tired: purple haze slowly breathing (softened) */
      .mood-bg-tired {
        background:
      ...
      /* anxious: warm orange flickering (softened) */
      .mood-bg-anxious {
        background:
      ...
      /* angry: red heartbeat pulsing (softened) */
      .mood-bg-angry {
        background:
    ...
    slots.inject('shell.overlay', () => slots.register(
      ...
      (props) => {
        ...
        const sessionId = useSessions === undefined ? undefined : useSessions((s) => s.current)
        const [mood, setMood] = React.useState(null)
        const [prevMood, setPrevMood] = React.useState(null)
        ...
        React.useEffect(() => {
          host.call('mood:set', { mood: mood === null ? null : mood }).catch(() => {})
        }, [mood])

        const runAnalysis = () => {
          if (!sessionId) return
          host.call('mood:analyze', { sessionId: sessionId })
            .then((res) => {
              if (!res || typeof res !== 'object' || !res.mood) return
        ...
        React.useEffect(() => {
          ...
          runAnalysis()
          return ctx.interval(runAnalysis, 2500)
        }, [auto, sessionId])
        ...
        return React.createElement('div', { className: 'mood-widget' },
          renderLayer(prevMood, true),
          renderLayer(mood, false),
          open ? React.createElement('div', { className: 'mood-panel' },
            React.createElement('div', { className: 'mood-panel-title' },
              current === null
                ? '当前情绪：无'
                : '当前情绪：' + current.label + (auto ? '（自动）' : '（手动）')
            ),
            React.createElement('button', {
              className: 'mood-row' + (auto ? ' mood-row-active' : ''),
              onClick: () => setAuto(!auto),
            },
              React.createElement('span', null, '🤖 自动感知 ' + (auto ? '开' : '关'))
            ),
            Object.keys(MOODS).map((key) => {
              ...
              return React.createElement('button', {
                ...
                React.createElement('span', { className: 'mood-dot', style: { background: m.color } }),
                React.createElement('span', null, m.emoji + ' ' + m.label)
              )
            }),
            React.createElement('button', { className: 'mood-row', onClick: clear },
              '✨ 清除情绪'
            ),
            React.createElement('div', { className: 'mood-panel-note' },
              '📝 当前情绪会注入系统提示词，AI 每次回复都能感知'
            )
          ) : null,
          React.createElement('button', {
            className: 'mood-orb',
            ...
            React.createElement('span', { className: 'mood-orb-core', style: coreStyle }),
            React.createElement('span', { className: 'mood-orb-ripple', style: rippleStyle }),
            React.createElement('span', { className: 'mood-orb-ripple', style: rippleStyle }),
            React.createElement('span', { className: 'mood-orb-ripple', style: rippleStyle })
          )
        )
